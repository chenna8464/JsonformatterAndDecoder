...
describe("csvToJson", () => {
  it("parses headers, quoted cells, and coerces types", () => {
    const rows = csvToJson('name,age,active,note\n"Lin, MD",42,true,"said ""hi"""\nAda,36,false,');
    expect(rows).toEqual([
  ...
  it("rebuilds nested objects from dot-path headers", () => {
    expect(csvToJson("name,meta.role\nAda,eng")).toEqual([{ name: "Ada", meta: { role: "eng" } }]);
  });
...
describe("queryJson", () => {
  it("resolves dot paths and indexes", () => {
    expect(queryJson(sample, "settings.rateLimit")).toEqual([{ path: "$.settings.rateLimit", value: 100 }]);
    expect(queryJson(sample, "endpoints[1].name")).toEqual([{ path: "$.endpoints[1].name", value: "patch" }]);
  });

  it("supports wildcards over arrays and objects", () => {
    expect(queryJson(sample, "endpoints[*].method").map((m) => m.value)).toEqual(["GET", "PATCH"]);
    expect(queryJson(sample, "settings.*").map((m) => m.value)).toEqual([100]);
  });

  it("supports filters", () => {
    expect(queryJson(sample, "endpoints[?auth=true].name").map((m) => m.value)).toEqual(["get"]);
    expect(queryJson(sample, "endpoints[?method!=GET].name").map((m) => m.value)).toEqual(["patch"]);
  });
...
describe("setAtPath", () => {
  it("immutably updates nested values including array elements", () => {
    const updated = setAtPath(sample, ["endpoints", 0, "method"], "POST") as typeof sample;
